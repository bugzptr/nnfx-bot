...
default_config_for_fallback = {
    ...
    "backtest_min_trades_for_ranking": 3, # Lowered for easier testing
    ...
    "scoring": { "win_rate_weight": 0.25, "profit_factor_weight": 0.25, "return_pct_weight": 0.20,
                 ...
                 "profit_factor_inf_score": 50.0, "sharpe_ratio_inf_score": 50.0}, # Ensure floats
    "signal_confidence": { "min_recent_data_for_confidence": 5, "default_confidence": 0.5, # Increased min_recent_data
...
class StrategyConfig:
    ...
    def _load_config(self) -> Dict[str, Any]:
        if not self.config_path.exists():
            ...
            return default_config_for_fallback.copy() # Return a copy
        try:
    ...
    def get(self, key_path: str, default_val_override: Any = None) -> Any:
        ...
        try:
            ...
            # logger.debug(f"Config key '{key_path}' not found in loaded params. Checking fallback.")
            ...
            try:
                ...
                # logger.debug(f"Using fallback default for '{key_path}': {current_level_fallback}")
                return current_level_fallback
            except (KeyError, TypeError):
                # logger.debug(f"Config key '{key_path}' not in fallback. Using override default: {default_val_override}")
...
class BitgetAPI:
    def __init__(self, api_key: str = "", secret_key: str = "", passphrase: str = "", sandbox: bool = True):
        ...
        self.rate_limit_delay = 0.25 # Slightly increased default delay
    ...
    def _generate_signature(self, timestamp: str, method: str, request_path: str, query_string: str = "", body_string: str = "") -> str:
        ...
        # Bitget signature: timestamp + method + requestPath + (queryString if GET else bodyString)
        # Ensure queryString starts with '?' if not empty, or is empty.
        # Ensure bodyString is the raw JSON string if POST.
        # The official SDKs might have more nuance for specific endpoints.
        # For GET with params, queryString is typically `?key1=value1&key2=value2`
        # For POST with JSON body, bodyString is the JSON payload.
        
        # Simplified: Bitget often uses timestamp + method + requestPath + body for POST,
        # and for GET, it might be timestamp + method + requestPath + sorted_query_params_string
        # The official doc for v1 spot is timestamp + method + requestPath + body (even for GET, body is empty string)
        # Let's stick to the common interpretation for v1:
        message_to_sign = timestamp + method.upper() + request_path
        if body_string: # For POST/PUT with body
            message_to_sign += body_string
        elif query_string and method.upper() == "GET": # For GET, if API needs query in signature (some do, some don't for v1)
             # Bitget v1 generally doesn't include query string in signature base string for GET
             pass
    ...
    def _get_headers(self, method: str, request_path: str, query_string: str = "", body_string: str = "") -> Dict[str, str]:
        ...
        headers = {
            ...
            "ACCESS-SIGN": self._generate_signature(timestamp, method, request_path, query_string, body_string),
            "ACCESS-TIMESTAMP": timestamp,
            "ACCESS-PASSPHRASE": self.passphrase,
            "Content-Type": "application/json;charset=UTF-8", # Standard content type
            "locale": "en-US"
        }
    ...
    def get_klines(self, symbol: str, granularity: str = "4H", limit: int = 500) -> pd.DataFrame:
        ...
        # Filename should be FS-safe and unique for symbol+granularity
        safe_symbol_fname = api_symbol_for_request.replace('/', '_') # Just in case symbol has /
        ...
        # Build query string for potential inclusion in signature if API required it for GET (Bitget v1 usually doesn't)
        # query_str_for_sig = "&".join([f"{k}={v}" for k,v in sorted(params.items())]) # Example if needed

        max_r, df_out = 3, pd.DataFrame()
        for attempt in range(max_r):
            headers = self._get_headers("GET", request_path, body_string="") # Empty body for GET
            try:
                ...
                if str(api_data.get('code')) == '00000' and isinstance(api_data.get('data'), list):
                    ...
                    # Bitget candle data: [timestamp, open, high, low, close, base_volume, quote_volume]
                    ...
                    if str(err_code) == '40309': return df_out # Delisted, don't retry
                    if attempt < max_r - 1: time.sleep(1 + 2**attempt + np.random.rand()) # Exponential backoff with jitter
    ...
    def get_all_tickers_data(self) -> List[Dict[str, Any]]: # Same structure
        request_path = "/api/spot/v1/market/tickers"
        try:
            time.sleep(self.rate_limit_delay)
            headers = self._get_headers("GET", request_path, body_string="")
...
class NNFXIndicators: # Same, uses config
...
def _backtest_worker_process(api_config_dict: Dict, strategy_config_path_str: str, symbol: str) -> Dict:
    ...
    # Setup basic logging for worker if needed, but it won't go to main process log file easily.
    # print(f"WORKER [{os.getpid()}] Starting for {symbol}") # For simple stdout feedback from worker
    try:
        ...
        # print(f"WORKER [{os.getpid()}] Finished {symbol}. Trades: {result.get('total_trades', 0)}")
        return result
    except Exception as e_w:
        # print(f"WORKER [{os.getpid()}] CRITICAL ERROR for {symbol}: {e_w}", file=sys.stderr)
        return {"symbol": symbol, "error": f"Worker process unhandled exception: {e_w}"}
...
class DualNNFXSystem:
    def __init__(self, bitget_api_instance: BitgetAPI, config_instance: StrategyConfig):
        ...
        self.indicators_instance = NNFXIndicators(config_instance) # Renamed to avoid conflict
        self.indicator_calc_errors_tracker = {} 

    def _safe_calc_ind(self, idx, log_name_prefix, func, *args): # Simplified safe calc
        try:
            res = func(*args)
            # Check for all NaNs which can indicate an issue in `ta` or data.
            ...
            if "Tuple" in str(func.__annotations__.get('return','')): # Heuristic for tuple output
                 nan_s = pd.Series(np.nan, index=idx, dtype=float)
                 # Count expected series in tuple based on annotation or common cases
                 num_series_expected = 2 # Default for Klinger, Chandelier
    ...
    def calculate_indicators(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        # ... (calculate_indicators logic - largely similar to previous full code, using self.indicators_instance and _safe_calc_ind)
        ...
        if not all(c in data.columns for c in req_cols):
            ...
            # Add NaN cols for all expected indicators if input is bad
        ...
        idx = data.index
        ind = self.indicators_instance # Alias for brevity
        ...
        data['atr'] = self._safe_calc_ind(idx, f"[{symbol}] ATR-Risk", ind.atr, data['high'], data['low'], data['close']) # Uses default risk period
        return data


    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame: # Same, uses config for W%R threshold
        df = data.copy()
        # Ensure all indicator columns used in signal logic exist, filling with NaN if created by error handling
        # NaNs in conditions will mostly evaluate to False, which is often desired (no signal if indicator failed).
        ...
        wpr_thresh = self.config.get("indicators.williams_r_threshold", -50) # Get threshold from config
    ...
    def backtest_pair(self, symbol: str) -> Dict: # Same, uses config
        cfg = self.config 
        ...
        for _, candle_row in df_signals.iterrows(): # Loop through candles
            # Entry Logic (simplified for brevity, same as before)
                    ...
                    if candle_row.get('long_signal', False):
                        current_pos = {'type': 'long', 'entry_price': price, 'entry_time': candle_row.name, 
                         ...
                         current_pos = {'type': 'short', 'entry_price': price, 'entry_time': candle_row.name, 
                                       'stop_loss': price + sl_mult * atr, 'take_profit': price - tp_mult * atr, 
                                       'atr_at_entry': atr, 'position_size': (equity * risk_val) / (sl_mult * atr)}
            # Exit Logic (simplified for brevity, same as before)
                ...
                if exit_now:
                    ...
                    equity = max(0, equity + pnl_usd) # Ensure equity doesn't go negative
        ...
        # Result packaging (simplified for brevity, same as before)
        ...
        if not df_trades.empty: # Ensure timestamps are timezone-naive before calculations
        ...
        if not equity_hist: equity_hist.append({'timestamp': df_signals.index[0] if not df_signals.empty else pd.Timestamp.now(), 'equity': 10000.0, 'in_position': False})
        ...
        pf = wins_df['pnl_r'].sum()/abs(losses_df['pnl_r'].sum()) if not losses_df.empty and abs(losses_df['pnl_r'].sum())>1e-9 else (np.inf if not wins_df.empty else 0.0)
    ...
    def _calculate_sharpe_ratio(self, trades_df: pd.DataFrame, equity_curve_data: List[Dict], rfr_annual_pct: float = 0.0) -> float: # Same
    ...
    def _calculate_sortino_ratio(self, trades_df: pd.DataFrame, equity_curve_data: List[Dict], target_r_trade: float = 0.0) -> float: # Same
        if trades_df.empty or trades_df['pnl_r'].isnull().all() or len(trades_df['pnl_r'].dropna())<2: return 0.0
    ...
    def scan_pairs(self, symbols: List[str], save_results: bool = True, max_workers: Optional[int] = None) -> pd.DataFrame: # Same
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i, fut in enumerate(as_completed(futures_map)):
                ...
                try:
                    res = fut.result()
                    # logger.info(f"Main: Completed {s_name} ({i+1}/{len(symbols_map)}).") # Too verbose for many pairs
                    min_tr = self.config.get("backtest_min_trades_for_ranking",3)
                    # Handle results that are valid but might not meet trade count for ranking
                    if 'error' not in res or (res.get('error') == 'No trades' and res.get('total_trades',0) == 0):
                        if res.get('total_trades',0) >= min_tr :
                            # Append all calculated metrics for scoring and reporting
                            res['score'] = self._calculate_score(res) # Calculate score based on full result dict
                            results.append(res) # Append the whole dict
                        else: # Not enough trades, but no other error
                            failed[s_name] = f"Trades < {min_tr} ({res.get('total_trades',0)})"
                    else: # Explicit error from worker or result dict
                        failed[s_name] = res.get('error', 'Unknown worker error')
                except Exception as e_fut:
                    logger.error(f"Exception for {s_name} future: {e_fut}", exc_info=False)
                    failed[s_name] = f"Future processing error: {e_fut}"
        
        logger.info(f"Scan done. Ranked: {len(results)}, Failed/Skipped: {len(failed)}.")
        # Convert list of full result dicts to DataFrame for final processing
        df_res = pd.DataFrame(results) # This will have all keys from result dicts
        if not df_res.empty:
            # Select only columns needed for final report/ranking to avoid issues with complex objects like DataFrames in 'trades'
            ...
            # Ensure all these columns exist, fill with default if not (e.g. if a result was partial)
            for col_report in cols_for_final_df:
                if col_report not in df_res.columns: df_res[col_report] = np.nan if 'pct' in col_report or 'ratio' in col_report or 'score' in col_report else 0
    ...
    def _save_scan_artifacts(self, df_ranked: pd.DataFrame, symbols_scanned: List[str], failed_pairs_info: Dict[str, str]): # Same
        ...
        with open(res_dir/f"scan_summary_{ts}.txt",'w') as f:
            ...
            if not df_ranked.empty:
                f.write("\nTop Performers:\n")
                for _,r in df_ranked.head(min(10,len(df_ranked))).iterrows(): f.write(f"  {r['symbol']:<12} Score:{r.get('score',0):.2f} WR:{r.get('win_rate',0):.1%} PF:{r.get('profit_factor',0):.2f}\n")
            if failed_pairs_info: f.write("\nFailed/Skipped:\n"); [f.write(f"  - {s}: {reason}\n") for s,reason in failed_pairs_info.items()]
        logger.info(f"Summary: {res_dir}/scan_summary_{ts}.txt")

    def _calculate_score(self, result_dict: Dict) -> float: # Same, uses config
        ...
        s_wr = wr*100; s_pf = min(pf*20,100) if np.isfinite(pf) else (cfg_s.get("profit_factor_inf_score",50) if pf>0 else 0)
        s_ret=min(max(ret,-100),100); s_nt=min(nt*0.5,100)
        s_sr = min(max(sr*25,-100),100) if np.isfinite(sr) else (cfg_s.get("sharpe_ratio_inf_score",50) if sr>0 else -50)
    ...
    def get_current_signals(self, symbols: List[str]) -> pd.DataFrame: # Same, uses config
        ...
        for s in symbols:
            try:
                ...
                last,curr = df_s.iloc[-2],df_s.iloc[-1] # Last closed, current forming
                sig,conf="NONE",0.0
                hist_for_conf = df_s.iloc[-(cfg_sc.get("min_recent_data_for_confidence",5)+1) : -1] # Use last N closed candles for confidence
                ...
                time.sleep(0.05) # Shorter delay for signals
    ...
    def _calculate_signal_confidence(self, recent_df: pd.DataFrame, sig_type: str, cfg_c: Dict) -> float: # Same
        ...
        # Use .mean() on boolean series (True=1, False=0)
        s_sup = recent_df.get(sig_col,pd.Series(False)).mean()
        s_trend = (recent_df.get('system_a_baseline',pd.Series(0)) == direction).mean()
        s_mom = (recent_df.get('system_a_confirmation',pd.Series(0)) == direction).mean()
        s_vol = (recent_df.get('system_a_volume',pd.Series(0)) == direction).mean()
        ...
        try:
            with pd.ExcelWriter(fn,engine='openpyxl') as w:
                ...
                if not rankings_df.empty:
                    pd.DataFrame({"Metric":["Total Ranked","Avg WR","Avg PF"], "Value":[len(rankings_df),f"{rankings_df['win_rate'].mean():.1%}",f"{rankings_df[np.isfinite(rankings_df['profit_factor'])]['profit_factor'].mean():.2f}"]}).to_excel(w,sheet_name="Summary",index=False)
...
def run_comprehensive_analysis(api_cfg_dict: Dict, main_strat_cfg: StrategyConfig, 
                               ...
                               max_w: Optional[int] = None) -> Dict: # Refined symbol selection
    ...
    if symbols_override:
        ...
        logger.info(f"Using override list: {len(symbols_to_scan)} symbols.")
    else:
        logger.info("Fetching all tickers for dynamic selection by volume...")
        all_t = api_inst.get_all_tickers_data()
        if all_t:
            logger.info(f"DEBUG: Total tickers fetched: {len(all_t)}")
            if all_t: logger.debug(f"DEBUG: Raw ticker sample [0]: {all_t[0] if all_t else 'N/A'}")

            usdt_t_init = [tk for tk in all_t if tk.get('symbolId','').upper().endswith('USDT')]
            logger.info(f"DEBUG: Found {len(usdt_t_init)} USDT tickers initially.")
            ...
            if main_strat_cfg.get("filter_by_major_bases_for_top_volume", True):
                ...
                usdt_t_base_filtered = [tk for tk in usdt_t_init if tk.get('symbolId','').upper()[:-4] in maj_b_set]
                ...
                elif usdt_t_init: logger.warning("DEBUG: Base filtering resulted in empty list. Check config.")
            
            def get_vol(ticker_d): # Volume extraction
                # Bitget often uses 'usdtVol' for spot tickers volume in USDT
                keys_order = ['usdtVol', 'quoteVol', 'quoteVolume', 'vol'] # Prioritized keys
                for k_vol in keys_order:
                    v_str = ticker_d.get(k_vol)
                    if v_str is not None: # Found a key
                        try: return float(str(v_str))
                        except (ValueError,TypeError): continue # Try next key if this one is bad
                # logger.debug(f"Volume not found or invalid for {ticker_d.get('symbolId')}, returning 0.")
                return 0.0

            # Sort by volume
            # Create list of (volume, ticker_dict) for stable sort if volumes are identical or 0
            ...
            # Log top 5 after sort for verification
            log_top_5_vols = [{'s':t.get('symbolId'), 'v':get_vol(t)} for t in sorted_usdt_t[:5]]
            logger.info(f"DEBUG: Top 5 USDT tickers post-sort by volume: {log_top_5_vols}")
            if not any(item['v']>0 for item in log_top_5_vols): # If top 5 all have 0 volume
                logger.warning("DEBUG: Top sorted tickers all show 0 volume. Volume key extraction might be incorrect for this API response or data is poor.")

            top_n_cfg = main_strat_cfg.get("top_n_pairs_by_volume_to_scan", 8)
            final_t_list = sorted_usdt_t[:top_n_cfg]
            symbols_to_scan = [tk.get('symbolId','').replace('_SPBL','') for tk in final_t_list if tk.get('symbolId')]
            symbols_to_scan = [s for s in symbols_to_scan if s] # Remove empty strings
            logger.info(f"Selected Top {len(symbols_to_scan)} symbols for scan: {symbols_to_scan}")
        else: # all_tickers was empty
    ...
    if not df_ranks.empty:
        ...
        top_sigs = df_ranks['symbol'].head(5).tolist()
        if top_sigs:
            df_curr_sigs = system_inst.get_current_signals(top_sigs)
        ...
        else: # No ranked pairs to get signals for
        ...
        system_inst.export_detailed_results(pd.DataFrame(),pd.DataFrame()) # Save empty report
...
if __name__ == "__main__":
    ...
    try:
        # --- File Logging ---
        p_logs = Path("logs"); p_logs.mkdir(exist_ok=True)
        path_log_file = p_logs / f"system_run_{ts_run}.log"
        h_file_log = logging.FileHandler(path_log_file)
        h_file_log.setLevel(logging.INFO) # File log level
        h_file_log.setFormatter(logging.Formatter('%(asctime)s-%(name)s-%(levelname)s-%(message)s'))
        logging.getLogger().addHandler(h_file_log) # Add to root
        # Adjust console handler if already set by basicConfig
        for h_console in logging.getLogger().handlers:
            if isinstance(h_console, logging.StreamHandler) and not isinstance(h_console, logging.FileHandler):
                h_console.setLevel(logging.INFO) # Set console to INFO
        
        logger.info(f"NNFX System Run ID: {ts_run}")
        logger.info(f"Strategy Config: {strategy_config_global.config_path}")
        logger.info(f"File Log (INFO+): {path_log_file}")
        logger.info(f"Console Log (INFO+) active.")

        # --- API Config ---
        cfg_api = {}
        path_api_cfg = Path("config/api_config.json")
        if path_api_cfg.exists():
            try: 
                with open(path_api_cfg,"r") as f: cfg_api=json.load(f)
                logger.info("API credentials loaded.")
            except Exception as e_api: logger.error(f"API cfg load err {path_api_cfg}: {e_api}. No API keys.")
        else:
            logger.warning(f"API cfg {path_api_cfg} not found. Creating dummy. Public API only.")
            try:
                path_api_cfg.parent.mkdir(exist_ok=True, parents=True)
                with open(path_api_cfg,"w") as f_dum: json.dump({"api_key":"", "secret_key":"", "passphrase":"", "sandbox":True},f_dum,indent=4)
                logger.info(f"Dummy API cfg created: {path_api_cfg}. Please update.")
            except Exception as e_dum_api: logger.error(f"Dummy API cfg creation err: {e_dum_api}")
        
        # --- Run Analysis ---
        symbols_for_run = None # None = dynamic selection by volume
        # symbols_for_run = ['BTCUSDT', 'ETHUSDT'] # Example manual override
        
        cpus = os.cpu_count()
        num_proc_workers = max(1, cpus - 1 if cpus and cpus > 1 else 1)
        logger.info(f"Using up to {num_proc_workers} worker processes for scan.")

        output_analysis = run_comprehensive_analysis(
            cfg_api, strategy_config_global, symbols_override=symbols_for_run, max_w=num_proc_workers)
        
        if output_analysis.get('success'): logger.info("Main: Analysis OK.")
        else: logger.error(f"Main: Analysis FAILED: {output_analysis.get('error','Unknown')}")

    except Exception as e_main_run: logger.critical("MAIN EXECUTION BLOCK ERROR:", exc_info=True)
    finally:
